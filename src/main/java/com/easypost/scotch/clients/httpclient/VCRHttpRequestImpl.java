...
public class VCRHttpRequestImpl extends VCRHttpRequest implements VCRWebSocketRequest {

    private final HttpHeaders userHeaders;
    private final VCRHttpHeadersBuilder systemHeadersBuilder;
     ...
     */
    public VCRHttpRequestImpl(VCRHttpRequestBuilderImpl builder) {
        String method = builder.method();
        this.method = method == null ? "GET" : method;
        this.userHeaders = HttpHeaders.of(builder.headersBuilder().map(), VCRUtils.ALLOWED_HEADERS);
        this.systemHeadersBuilder = new VCRHttpHeadersBuilder();
    ...
    /**
     * Creates a VCRHttpRequestImpl from the given request.
     */
    public VCRHttpRequestImpl(VCRHttpRequest request, ProxySelector ps) {
        String method = request.method();
        if (method != null && !VCRUtils.isValidName(method))
        ...
        this.method = method == null ? "GET" : method;
        this.userHeaders = HttpHeaders.of(request.headers().map(), VCRUtils.VALIDATE_USER_HEADER);
        if (request instanceof VCRHttpRequestImpl) {
            // all cases exception WebSocket should have a new system headers
            this.isWebSocket = ((VCRHttpRequestImpl) request).isWebSocket;
            if (isWebSocket) {
                this.systemHeadersBuilder = ((VCRHttpRequestImpl)request).systemHeadersBuilder;
            } else {
                this.systemHeadersBuilder = new VCRHttpHeadersBuilder();
            }
        } else {
            VCRHttpRequestBuilderImpl.checkURI(requestURI);
        ...
        if (isWebSocket) {
            ...
            this.proxy = ((VCRHttpRequestImpl) request).proxy;
        } else {
            if (ps != null)
    ...
    public static VCRHttpRequestImpl newInstanceForRedirection(URI uri,
                                                                                  String method,
                                                                                  VCRHttpRequestImpl other,
                                                                                  boolean mayHaveBody) {
        return new VCRHttpRequestImpl(uri, method, other, mayHaveBody);
    }

    /** Returns a new instance suitable for authentication. */
    public static VCRHttpRequestImpl newInstanceForAuthentication(
            VCRHttpRequestImpl other) {
        VCRHttpRequestImpl
                request = new VCRHttpRequestImpl(other.uri(), other.method(), other, true);
        if (request.isWebSocket()) {
            VCRUtils.setWebSocketUpgradeHeaders(request);
        }
        return request;
    }

    /**
     * Creates a VCRHttpRequestImpl using fields of an existing request impl.
     * The newly created VCRHttpRequestImpl does not copy the system headers.
     */
    private VCRHttpRequestImpl(URI uri,
                            String method,
                            VCRHttpRequestImpl other,
                            boolean mayHaveBody) {
        assert method == null || VCRUtils.isValidName(method);
        this.method = method == null? "GET" : method;
    ...
    private HttpRequest.BodyPublisher publisher(VCRHttpRequestImpl other) {
    ...
    VCRHttpRequestImpl(String method, InetSocketAddress authority, VCRUtils.ProxyHeaders headers) {
    ...
    /**
     * Creates a VCRHttpRequestImpl from the given set of Headers and the associated
     * "parent" request. Fields not taken from the headers are taken from the
     * parent.
     */
    static VCRHttpRequestImpl createPushRequest(VCRHttpRequestImpl parent,
                                                                   HttpHeaders headers)
            throws IOException
    {
        return new VCRHttpRequestImpl(parent, headers);
    }

    // only used for push requests
    private VCRHttpRequestImpl(VCRHttpRequestImpl parent, HttpHeaders headers)
            throws IOException
    {
        ...
        this.proxy = null;
        this.userHeaders = HttpHeaders.of(headers.map(), VCRUtils.ALLOWED_HEADERS);
    ...
    VCRHttpHeadersBuilder getSystemHeadersBuilder() { return systemHeadersBuilder; }
